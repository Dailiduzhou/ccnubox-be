...
// NewClassData .
func NewClassData(cli *elastic.Client, l logger.Logger) (*ClassData, func(), error) {
	cleanup := func() {
		l.Info("closing Elasticsearch resources")
	}
	return &ClassData{
		cli: cli, logger: l,
	}, cleanup, nil
...
func (d ClassData) AddClassInfo(ctx context.Context, classInfos ...model.ClassInfo) error {
	...
	if err != nil {
		d.logger.Errorf("es: failed to bulk add class_info: %v", err)
		return fmt.Errorf("%w: %v", biz.ErrESAddClassInfo, err)
	}

	// 检查是否有失败的请求
	if bulkResponse.Errors {
		for _, failed := range bulkResponse.Failed() {
			d.logger.Errorf("es: failed to index class_info[%s]: %v", failed.Id, failed.Error)
		}
		return biz.ErrESAddClassInfo
	}
...
func (d ClassData) ClearClassInfo(ctx context.Context, xnm, xqm string) {
	...
	deleteResponse, err := d.cli.DeleteByQuery().
		...
		Do(ctx)
	if err != nil {
		d.logger.Errorf("es: failed to delete class_info[except (xnm:%v,xqm:%v)]: %v", xnm, xqm, err)
		return
	}
	d.logger.Infof("Deleted %d documents", deleteResponse.Deleted)
}
...
func (d ClassData) SearchClassInfo(ctx context.Context, keyWords string, xnm, xqm string, page, pageSize int) ([]model.ClassInfo, error) {
	...
	if err != nil {
		d.logger.Errorf("es: failed to search class_info[keywords:%v xnm:%v xqm:%v]: %v", keyWords, xnm, xqm, err)
		return nil, biz.ErrESSearchClassInfo
	}

	for _, hit := range searchResult.Hits.Hits {
		var classInfo model.ClassInfo
		if err := json.Unmarshal(hit.Source, &classInfo); err != nil {
			d.logger.Errorf("json unmarshal %v failed: %v", hit.Source, err)
			continue
		}
...
func (d ClassData) GetBatchClassInfos(ctx context.Context, year, semester string, page, pageSize int) ([]model.ClassInfo, int, error) {
	...
	if err != nil {
		d.logger.Errorf("es: failed to get all class_info[ year:%v semester:%v]: %v", year, semester, err)
		return nil, 0, biz.ErrESSearchClassInfo
	}
	// 处理结果

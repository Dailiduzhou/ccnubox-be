...
func newResource(serviceName string) (*resource.Resource, error) {
	// Keep application attributes schemaless when merging them with the SDK's
	// default resource. The SDK schema may change after a dependency upgrade;
	// assigning a fixed schema here would make resource.Merge reject otherwise
	// compatible resources and crash every service during startup.
